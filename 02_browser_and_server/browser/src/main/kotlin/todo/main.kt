...
class AddTodoResult(val result: Result<String, Unit>) : Msg()
...
fun update(msg: Msg, model: Model): Pair<Model, Cmd<Msg>?> {
    return when (msg) {
        ...
        Add -> {
            ...
            model.copy(newTodoName = "", todoList = model.todoList + newTodo) to todoCrate.add(newTodo, ::AddTodoResult)
        }
        ...
        is AddTodoResult -> {
            when (msg.result) {
                ...
                is Ok -> model to Cmd.alert("추가하였습니다.")
            }
        }
